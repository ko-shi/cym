...
namespace cym {
	...
	public:
		...
		void pushBack(std::int32_t data, const AdT &additional_data) {
			DataSetType dataset{ 4,additional_data};
			buffer_.reserve(buffer_.size() + 4 + sizeof(DataSetType));
			*static_cast<std::int32_t*>(static_cast<void*>(buffer_.end())) = data;
		...
		std::int32_t backAs32() const{
			return *static_cast<const std::int32_t*>(static_cast<const void*>(buffer_.end() - sizeof(DataSetType) - 4));
		}
